// Inline fakesc: renders text as small-caps via letter-spacing + uppercase + smaller size
#let fakesc(content) = text(tracking: 0.05em, upper(content))
...
#let resume(
  ...
  show link: it => {underline(stroke: link-line-settings.stroke, offset: link-line-settings.offset, text(fill: accent-color, it))}
  ...
  show heading.where(level: 2): it => [
    #block(breakable: false, above: heading-settings.above-spacing, below: heading-settings.below-spacing)[
      ...
      #row2col(text(size: heading-settings.section-title-size, weight: heading-settings.section-title-weight, fill: heading-settings.at("heading-color", default: black))[#parsed.title], if section_note != none { text(size: heading-settings.section-note-size, weight: heading-settings.section-note-weight, fill: heading-settings.at("heading-color", default: black))[#section_note] } else { [] })
  ...
  align(author-position)[
    #text(size: font-settings.author-font-size, weight: "bold", fill: author-info.at("name-color", default: black))[#author-info.name] \
    #v(1em)
    #par(leading: 0.6em)[
